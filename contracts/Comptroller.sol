...
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    /// @notice Emitted when Annex VAI rate is changed
    event NewAnnexVAIRate(uint oldAnnexVAIRate, uint newAnnexVAIRate);

    /// @notice Emitted when Annex VAI Vault rate is changed
    event NewAnnexVAIVaultRate(uint oldAnnexVAIVaultRate, uint newAnnexVAIVaultRate);
    ...
    function distributeVAIMinterAnnex(address vaiMinter, bool distributeAll) public {
        ...
        if (address(vaiController) != address(0)) {
            ...
            if (err == uint(Error.NO_ERROR)) {
                annexAccrued[vaiMinter] = vaiMinterAccrued;
    ...
    function releaseToVault() public {
        ...
        // releaseAmount = annexVAIVaultRate * deltaBlocks
        uint256 _releaseAmount = mul_(annexVAIVaultRate, deltaBlocks);
