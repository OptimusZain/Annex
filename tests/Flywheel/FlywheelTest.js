...
describe('Flywheel', () => {
  ...
  describe('updateAnnexSupplyIndex()', () => {
    ...
    it('should not update index on non-Annex markets', async () => {
      ...
      const speed = await call(comptroller, 'annexSpeeds', [mkt._address]);
      expect(speed).toEqualNumber(0);
      // vtoken could have no annex speed or ann supplier state if not in annex markets
  ...
  describe('claimAnnex', () => {
    ...
    it('should revert when a market is not listed', async () => {
      ...
      await expect(
        send(comptroller, 'claimAnnex', [a1, [cNOT._address]])
  ...
  describe('claimAnnex batch', () => {
    ...
    it('should revert when a market is not listed', async () => {
      const cNOT = await makeAToken({comptroller});
      await expect(
        send(comptroller, 'claimAnnex', [[a1, a2], [cNOT._address], true, true])
      ).rejects.toRevert('revert not listed market');
    });
  });
